...
const randomStyleIDGenerator = () => {
  let letters = Math.random()
    ...
    .substr(0, 2)
    .toUpperCase();
  let fourcode = Math.floor(
    Number.parseFloat(Math.random()).toPrecision(4) * 10000
  );
  let twocode = Math.floor(
    Number.parseFloat(Math.random()).toPrecision(2) * 100
  );
...
const generateReactShoe = (givenName) => {
  ...
  let generateReactName = () => {
    ...
    if (Math.random() >= 0.8) {
      ...
      let randomReactPrefix =
        reactPrefixsArray[Math.floor(Math.random() * reactPrefixsArray.length)];
      let randomReactSubPrefix =
        reactPrefixs[randomReactPrefix][
          Math.floor(Math.random() * reactPrefixs[randomReactPrefix].length)
        ];
      shoeName = `${randomReactPrefix} ${randomReactSubPrefix} ${shoeName}`;
    }
    //95 of the time there will be a variant added
    if (Math.random() >= 0.05) {
      let randomReactVariant =
        reactVariants[Math.floor(Math.random() * reactVariants.length)];
  ...
  shoe.discountPrice = generateRandomDiscountPrice(0.05, 0.4, shoe.price);
...
let generateZoomShoe = (givenName) => {
  ...
  let generateZoomShoeName = () => {
    ...
    let randomVarientVersion = `${
      zoomVariants[randomVariant][
        Math.floor(Math.random() * zoomVariants[randomVariant].length)
      ]
    }`;
    ...
    if (randomVariant == 'Pegasus') {
      ...
      let randomPegasusVariant =
        pegasusVariantArray[
          Math.floor(Math.random() * pegasusVariantArray.length)
        ];
      shoeName = `${shoeName} ${randomPegasusVariant}`;
      let randomPegasusVariantVersion =
        pegasusVariants[randomPegasusVariant][
          Math.floor(
            Math.random() * pegasusVariants[randomPegasusVariant].length
          )
        ];
  ...
  shoe.discountPrice = generateRandomDiscountPrice(0.05, 0.3, shoe.price);
  //passing generateRandomGender true allows for unisex option
  shoe.gender = generateRandomGender(false);
...
let generateRandomDataSet = () => {
  ...
  let shoeSetGenerators = [
    generateFreeRNShoeSet,
    generateReactShoeSet,
    generateZoomShoeSet
  ];

  let randomShoeSet = shoeSetGenerators[
    Math.floor(Math.random() * shoeSetGenerators.length)
  ]();
  randomShoeSet.forEach((shoe) => dataSet.push(shoe));

  let data = dataSet.reduce(
    (acc, shoe) => acc + JSON.stringify(shoe) + '\n',
    ''
  );
...
//pgWriteData creates JSON file with data to be stored into postgres DB
const pgWriteData = (amount) => {
  return new Promise((resolve, reject) => {
    const start = process.hrtime.bigint();
    let file = fs.createWriteStream(`../database-pg/data/dataSet.json`);
    let counter = amount;
    ...
    function drainer() {
      ...
      if (counter > 0) {
        file.once('drain', drainer);
      }
    }
  });
};

//getData generates data to be stored directly into MongoDB
...
module.exports = {
  getData: getData,
  writeData: writeData,
  pgWriteData: pgWriteData,
  generateRandomDataSet: generateRandomDataSet
};
